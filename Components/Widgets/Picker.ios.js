...
export default class PickerNB extends NativeBaseComponent {
    ...
    getLabel(props) {
        ...
        if (_.get(item, 'props.showValue', false) && _.isString(_.get(item, 'props.value')))
    ...
    renderHeader() {
        return (this.props.headerComponent) ? this.modifyHeader() : (<Header >
            <Button transparent onPress={() => {this._setModalVisible(false)}}>{this.props.backIcon ? <Icon name="ios-arrow-back" /> : 'Back'}</Button>
            <Title>
            {this.props.iosHeader ? this.props.iosHeader : 'Select One'}</Title>
        </Header>)
    }
